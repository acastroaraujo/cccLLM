...
```{r court-cohorts, message=FALSE, fig.height = 8, fig.width = 7, echo=FALSE}
library(tidyverse)
library(slider)
library(patchwork)
library(cccLLM)
...
p2 <- rulings |> 
  ...
  count(date) |> 
  mutate(avg = slide_index_dbl(n, date, mean, .before = months(3), .after = months(3))) |> 
  ggplot(aes(date)) + 
...
p1 / p2 + plot_layout(heights = c(6, 2), widths = c(7, 7))
```
